/*
  This algorithm is superb with time complexity of O(n*log(log(n))) and used to find prime numbers in a given range .
...
Segmented Seive can solve those problems, but time complexity would be same.
*/
...
{
  cout << 2 << " ";    
  for(int i = 3; i < n; i += 2)   // i+=2 as we know 2 is the only even prime number
    if(prime[i])
      cout<<i<<" ";
